# 一.組別:
### 三

# 二.系級班級:
### 資工一A

# 三.成員資訊:
### 王銘詳<br>翁智群<br>孔令喆

# 四.小專題題目:
### 中國暗棋

# 五.程式介紹(含 UML 圖、操作說明):

<img width="2816" height="1536" alt="Gemini_Generated_Image_ocrmy7ocrmy7ocrm" src="https://github.com/user-attachments/assets/08f799ce-474b-4303-a73a-cfc31e44d9c7" />

## 分成六個獨立的原始碼檔案，分別為piece.h、Constants.h、Board.h、piece.cpp、Board.cpp、main.cpp

### Constants.h : 基礎定義	定義遊戲通用的常數。包含棋子的顏色（紅、黑、無）與等級（帥到兵）。它是所有檔案的基礎，確保大家對「紅方」或「帥」的定義是一致的。

### Piece.h :	棋子規格書	宣告 Piece 類別。定義一顆棋子該有的屬性（它是誰、翻開了沒）與行為（翻面、取得名字）。

### Piece.cpp	: 棋子行為實作	負責處理棋子的具體細節。

### Board.h	: 棋盤管理員宣告	宣告 Board 類別。

### main.cpp :　負責與玩家互動。


# 六.程式如何安裝執行(圖文講解):
<img width="960" height="528" alt="image" src="https://github.com/user-attachments/assets/85737f6b-cf2e-4cdd-930c-270c098f4efa" />

### 輸入f (意旨flip)<br>
### 輸入行列翻開棋子
<img width="960" height="539" alt="image" src="https://github.com/user-attachments/assets/c4351f0e-33f7-4b61-a17c-1cdfc51db03c" />

### 輸入m (意旨move)<br>
### 輸入原本位置的行列<br>
### 輸入目標位置的行列



# 七.程式畫面截圖:
<img width="1919" height="1079" alt="image" src="https://github.com/user-attachments/assets/23273ac9-b993-4a2e-9ea5-a662701b7c32" />


# 八.分工資訊:
### 王銘詳:程式碼編寫<br>
### 翁智群:簡報製作


### 孔令喆:不知道 沒看過人

